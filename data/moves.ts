...
inversepower: {
	...
	// Like Stored Power, but reading the *target's* boosts
	basePowerCallback(pokemon, target) {
		let boostTotal = 0;
		const boosts = target.boosts;

		for (const stat in boosts) {
			// TS-safe indexing – mirror PS style
			const boost = (boosts as any)[stat] as number;
			if (boost > 0) boostTotal += boost;
		}

		// Stored Power formula: 20 + 20 × (# of positive stages)
  ...
  serratedspikes: {
    ...
    condition: {
      // Side condition that inflicts Bleeding to grounded switch-ins (tweak as your Bleeding rules require)
      onStart(side) {
  ...
  nuclearexplosion: {
    ...
    flags: {protect: 1, mirror: 1},
    selfdestruct: "always",
    // Deal bench damage as well after normal hit calc
    onAfterMove(pokemon) {
      // active + benched on both sides
      const sides = [pokemon.side, pokemon.side.foe];
      for (const s of sides) {
        for (const pkm of s.pokemon) {
          // include benched and active, but skip fainted
          if (!pkm || pkm.fainted) continue;
          const dmg = Math.floor(pkm.baseMaxhp * (this.random(1, 51) / 100));
          if (dmg <= 0) continue;
          // don't re-hit the user here (selfdestruct will KO them anyway)
          if (pkm === pokemon) continue;
          this.damage(dmg, pkm, pokemon, this.dex.getActiveMove('nuclearexplosion'));
  ...
  radiantspear: {
    ...
    onEffectiveness(typeMod, target, type) {
      if (!target) return;
      // If this call is evaluating the target's Dark type, force +1
  ...
  sylvanlament: {
    ...
    onAfterHit(target, pokemon, move) {
      if (!target || target.fainted) return;
      const eff = this.dex.getEffectiveness(move.type, target.types[0]) +
                  (target.types[1] ? this.dex.getEffectiveness(move.type, target.types[1]) : 0);
      if (eff > 0) {
  ...
  divinerage: {
    name: "Divine Rage",
    shortDesc: "Self: gain a condition. +1 Atk every time you're hit (multi-hit gives multiple boosts).",
    type: "Light",
    category: "Status",
    accuracy: true,
    basePower: 0,
    ...
    condition: {
      ...
      onDamagingHit(_damage, target, _source, _move) {
...
/*
  bloodoath: {
    ...
    onTryHit(target, pokemon) {
      ...
      // Apply a slot condition so it persists when target switches
      target.side.addSlotCondition(target, 'bloodoath', pokemon);
      this.add('-message', `${pokemon.name} bound ${target.name} by a blood oath!`);
    },
    // Slot condition: mirrored damage 1/3 to the other bound member (tunable)
    slotCondition: {
      // store the binder
      onStart(target, source) {
        this.add('-start', target, 'Blood Oath');
        // Keep a reference to the binder's position
      ...
      onDamage(damage, target, source, effect) {
        ...
        // mirror a portion of damage to the binder if still around
        const sid = this.effectState.binderSid;
        const pos = this.effectState.binderPos;
        const binder = this.getAtSlot([sid, pos] as unknown as Pokemon);
        if (binder && !binder.fainted && damage > 0) {
          const spill = Math.floor(damage / 3);
          if (spill > 0) this.damage(spill, binder, target, this.dex.conditions.get('bloodoath'));
        }
      },
      onEnd(target) {
        this.add('-end', target, 'Blood Oath');
      },
      // persist through switch by virtue of being a slot condition
    },
    target: "normal",
  },
*/
  ...
  talktothehand: {
	num: -1002, // any unused/custom number
	accuracy: true,
	basePower: 0,
	category: "Status",
	...
	condition: {
		...
		onTryHit(target, source, move) {
			...
			// Standard Protect bypass handling
			if (!move.flags['protect'] || move.category === 'Status') {
				if (['gmaxoneblow', 'gmaxrapidflow'].includes(move.id)) return;
				if (move.isZ || move.isMax) target.getMoveHitData(move).zBrokeProtect = true;
				return;
			...
			if (lockedmove && source.volatiles['lockedmove'].duration === 2) {
				// Outrage-style counter reset
				delete source.volatiles['lockedmove'];
			}

			// === Custom effect: Disable the move that hit into it for 3 turns ===
			if (move.id !== 'struggle') {
				if (!source.volatiles['disable']) {
					source.addVolatile('disable', target);
				}
				const disableVol = source.volatiles['disable'];
				if (disableVol) {
					disableVol.move = move.id;
					disableVol.duration = 3; // exactly 3 turns
				}
		...
		// If a Z/Max move breaks through and actually hits, also disable it
		onHit(target, source, move) {
			if (!move || move.category === 'Status' || move.id === 'struggle') return;
			if (!move.isZOrMaxPowered) return;
  ...
  enchantingspin: {
    ...
    onAfterMoveSecondarySelf(pokemon) {
      ...
      for (const cond of ['spikes','toxicspikes','stealthrock','stickyweb','serratedspikes','gmaxsteelsurge','gasoline','twinvines']) {
        if (side.removeSideCondition(cond as ID)) {
          this.add('-sideend', side, this.dex.conditions.get(cond).name, '[from] move: Enchanting Spin', '[of] ' + pokemon.name);
        }
      }
    },
    target: "normal",
  ...
  pyrokinesis: {
	name: "Pyrokinesis",
	shortDesc: "For 5 turns, reflects Fire moves and halves Water damage to this side. (8 with Light Clay.)",
	type: "Fire",
	category: "Status",
	accuracy: true,
	basePower: 0,
	...
	condition: {
		...
		onSideStart(side) {
			...
			// Remember which side this condition belongs to
			(this.effectState as any).side = side;
		},
		...
		// Reflect damaging Fire moves back at the attacker
		onTryHit(target, source, move) {
			if (!move || move.category === 'Status') return;
			if (move.type !== 'Fire') return;
			const side = (this.effectState as any).side;
			// Only protect the side that has Pyrokinesis
			if (!target || target.side !== side) return;
			if (!source || source === target) return;

			this.add('-activate', target, 'move: Pyrokinesis');

			// Bounce the move: defender uses same move on original attacker
			(this.actions as any).useMove(move.id, target, source);
			return null; // block the original hit
		},

		// Halve Water-type damage taken by this side
		onAnyModifyDamage(damage, source, target, move) {
			...
			const side = (this.effectState as any).side;
			if (target.side !== side) return; // only protect our side

			// Optional: if you want crits to ignore it, you can skip on crit here

			// TS thinks `this` is just ConditionData; cast to any for Battle methods
			(this as any).debug?.('Pyrokinesis water damage halved');
			return (this as any).chainModify(0x800); // 0.5x
		},
	},
},
  ...
  ragnarokcry: {
	name: "Ragnarok Cry",
	shortDesc: "Clears all field effects before damage. If a terrain was removed, -1 SpD to target. Sound.",
	type: "Flying",
	category: "Special",
	accuracy: 100,
	basePower: 90,
	...
	onTryMove(attacker, defender, move) {
		...
		// If you want to be explicit about which terrains count for the debuff:
		// const terrains: ID[] = ['electricterrain', 'grassyterrain', 'psychicterrain',
		//   'mistyterrain', 'darkterrain', 'lightterrain'];
		// if (this.field.isTerrain(terrains)) {
		//   removedTerrain = true;
		//   this.field.clearTerrain();
		// }

		// --- Clear standard weathers ---
		for (const weather of [
			...
			if (this.field.isWeather(weather)) {
				...
				break; // only one real weather at a time
			}
		}

		// --- Clear pseudo-weathers / custom fields you use ---
		...
		// If a terrain was removed, drop target's SpD by 1
		if (removedTerrain) {
  ...
  dodgeball: {
	name: "Dodgeball",
	shortDesc: "50% OHKO. If it fails, user faints and the target's 4th slot becomes Dodgeball.",
	type: "Fighting",
	category: "Physical",
	accuracy: 50,
	basePower: 1, // it’s basically all-or-nothing
	pp: 5,
	priority: 0,
	ohko: false, // we flip this to true on success
	flags: {protect: 1, mirror: 1, contact: 1, metronome: 1},
	target: "normal",

	onTryHit(target, source, move) {
		// 50% chance to become an OHKO move
		if (this.randomChance(1, 2)) {
			move.ohko = true;
			return; // let normal OHKO handling proceed
		}

		// --- Catch branch: OHKO fails, foe "catches" the ball ---
		...
		// Try to teach Dodgeball into the target's 4th moveslot (index 3)
		const slot = 3;

		// setMove exists at runtime but isn't in typings, so cast to any
		const learned = (target as any).setMove
			? (target as any).setMove('dodgeball', slot)
			: null;
		...
		// Prevent the original hit from dealing any damage
		return null;
	},
},
  ...
  runecleave: {
    ...
    onBasePower(basePower, attacker, defender) {
      if (defender?.status === 'frb' || defender?.volatiles['frostbite']) {
        return this.chainModify([13, 10]); // 1.3x
      }
    },
    secondary: {chance: 30, status: 'frb'}, // assumes custom Frostbite id 'frb'
    target: "normal",
  },
  ...
  drako: {
    ...
    onTryMove(attacker, target, move) {
      ...
      const req = ['atk','def','spa','spd','spe'] as const;
      let ok = true;
      for (const stat of req) if ((b[stat] || 0) < 1) ok = false;
  ...
  gammaray: {
    ...
    onAfterHit(target) {
      if (!target || target.fainted) return;
      if (this.randomChance(3, 10)) {
        if (this.toID('radioactive') in this.dex.abilities.all()) {
          target.setAbility('Radioactive');
          this.add('-ability', target, 'Radioactive', '[from] move: Gamma Ray');
        } else {
          // Fallback: apply a volatile if you prefer to avoid overwriting abilities
  ...
  disorientingbite: {
    ...
    shortDesc: "Combines Dark effectiveness. 25% confuse.",
    type: "Psychic",
    category: "Physical",
    accuracy: 100,
    basePower: 80,
    ...
    secondary: {chance: 25, volatileStatus: 'confusion'},
    target: "normal",
  },
  ...
  ultimateslam: {
	...
	type: "Normal", // will be overridden each use
	category: "Physical",
	accuracy: 65,
	basePower: 150,
	pp: 5,
	...
	// ... however you're randomizing the type goes here ...

	onAfterSubDamage(damage, target, source, move) {
		if (!damage || !target || target.fainted) return;

		const overkill = damage - target.lastDamage / 1; // attempt to capture overflow – fallback below
		let spill = 0;

		// safer calculation: if the hit reduced to 0, approximate spill by (-hp)
		if (target.hp <= 0) {
